  ...
  useEffect(() => {
    ...
    const fetchPublicData = async () => {
      try {
        ...
        if (data) {
          ...
          setError('Produção não encontrada ou o link é inválido.');
        }
      } catch (e) {
        console.error(e);
   ...
   const renderContent = () => {
    ...
    if (!production || shootingDays.length === 0) {
        return (
             <div className="flex flex-col items-center justify-center min-h-[60vh]">
                <div className="text-center p-8 max-w-md bg-card border rounded-lg">
                    <h1 className="mt-4 text-2xl font-bold">{production?.name}</h1>
                    <p className="mt-2 text-muted-foreground">Nenhuma Ordem do Dia foi adicionada a esta produção ainda.</p>
                </div>
            </div>
        );
    }
