        ...
        firestoreApi.getPublicProduction(productionId)
            .then(fetchedData => {
                if (fetchedData) {
                    ...
                    setData({
                        production: fetchedData,
                        days: processedDays,
                    });
                } else {
                    toast({ variant: 'destructive', title: 'Produção não encontrada', description: 'O link pode estar incorreto ou a produção não é mais pública.' });
                }
            })
            .catch(error => {
                const errorTyped = error as { code?: string, message: string };
                toast({ variant: 'destructive', title: 'Erro ao carregar', description: <CopyableError userMessage="Não foi possível carregar os dados da produção." errorCode={errorTyped.code || errorTyped.message} />});
            })
        ...
        <div className="flex flex-col min-h-screen w-full bg-background">
            <header className="sticky top-0 z-10 flex h-[60px] items-center gap-4 border-b bg-background/95 backdrop-blur-sm px-6">
                ...
                <div className="ml-auto flex items-center gap-4">
                    <h1 className="text-xl font-bold">Visualização Pública</h1>
                    <Button asChild>
            ...
            <main className="flex-1 p-4 sm:p-6 md:p-8">
                ...
                <div className="space-y-4">
                    {days.length > 0 ? (
                        ...
                        <p className="text-center text-muted-foreground">Nenhuma Ordem do Dia disponível para visualização.</p>
                    )}
                </div>
            </main>
            <AppFooter />
